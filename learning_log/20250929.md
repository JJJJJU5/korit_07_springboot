...
1. 관심사 분리(Seperation of Concerns) : 
  - 데이터 처리(Model), 화면 표시(View), 요청 처리(Controller)가 명확하게 분리되어 있어, 각 부분을 독립적으로 개발하고 수정할 수 있음. 
    - 보여주는 부분이 잘못되었다면 -> View 수정
    - DB에 가져오지를 못한다 -> Repository or Service 수정 
    - 요청 엔드포인트가 잘못되었다 -> Controller 수정 
2. 높은 재사용성 : 
  동일한 Model(비지니스 로직)을 사용하여 여러 View(웹페이지, 모바일 앱, REST API 등)을 만들 수 있다.(사이즈를 줄여서 모바일 버전 웹 페이지를 제작한다든지, 회원 가입 창을 재활용해서 로그인 창으로 쓴다든지 등으로)
3. 유지 보수의 용이성 :
  - 화면 디자인이 변경되어도 Controller나 Model 코드는 거의 수정할 필요가 없다.
  반대로, 비지니스 로직이 바뀌어도 View에는 미치는 영향이 최소화된다.
  - 지금 카카오톡 생각하면 View가 바뀌었다고 볼 수 있다. 하지만 DB에 프로필 사진들은 다 있을 것 . 가장 최근 id를 기준으로 프로필 사진에, 나머지 사진들을 인스타그램식으로 고쳐놨다고 볼 수 있다. 즉 DB에 있는 다른 부분들의 코드들의 수정은 크게 일어나지 않았을 것이라고 볼 수 있다.
4. 테스트 용이성 :
  - 각 구성 요소가 독립적이기 때문에 단위 테스트를 쉽게 작성하고 수행할 수 있다.(GET 요청만 확인한다든지 등으로 금요일에 작성해봤다.) 특히 Controller와 Model의 비지니스 로직은 View와 독립적으로 테스트 가능하다(postman으로, HeidiSQL로 CRUD 테스트를 마무리 지었다. 화면은 아예 없는데도).
5. 병렬 개발 가능 : 
...
3. Controller는 요청을 받고 Model을 호출하여 View를 선택하는 중재자 역할을 한다.
  - View에서 요청(Request) -> Controller 내부의 코드에서 Service 호출 -> Service가 Repository를 호출하여 DB 관련 정보 습득 후 데이터 처리 -> Controller로 보내주면 -> Controller가 적절한 .html 문서를 응답(Response). / ResponseEntity를 사용 
4. Model은 비지니스 로직과 데이터를 담당(Entity - Repository - Service)

# Spring Initializr를 통해서 Cardatabase_4


# 프로젝트 만든 후에 실행 여부를 따졌을 때 실행이 되는 시점을 기준으로 한다.
1. Car / CarRepository를 만들었다.
  - 그런데 CarRepository 부분에서 JpaRepository를 상속 받으려고 하니 검색이 안되었다.
  `implementation 'org.springframework.boot:spring-boot-starter-data-jpa'` 이 부분이 build.gradle에 추가 되지 않아서이다. 추가하고 코끼리를 누르면 검색이 된다
  - 여전히 오류가 발생 -> Car field로 Owner가 있는데 현재 Owner Entity가 없어서.
2. Owner / OwnerRepository를 생성했다.
  - 그리고 실행시켰더니 실행이 되지 않음.
  - application.properties에 관련 설정들이 들어가지 않아서 , build.gradle에 mariadb 관련 의존성 추가를 하지 않아서 이다.
    - HeidiSQL 켜서 `SELECT * FROM car;` 했을 때 row = 0 인 테이블이 생성 완료 되어있어야 함.
  - 하지만 Controller 파트를 작성하지 않았기 때문에 HeidiSQL에서는 데이터를 확인 및 추가가 가능하지만 postman을 통해서는 불가능하다는 것도 확인.
3. 이상의 과정에서 생긴 문제를 보아 MVC 패턴을 만들어야 postman에서 CRUD를 할 수 있다는 걸 알 수 있다. 그러면 Model 파트에서 만든 Entity와 Repository를 제외하고 Service 파트도 만들어야하고, Controller 파트에 해당하는 Controller 클래스도 만들어야 된다.
  - 루트 프로젝트 패키지에서 service 패키지를 생성하고 -> CarService를 생성한다.

4. 그러면 이제 페이지를 보여줄 수 있도록해야하는데, 이걸 React와 연결할거니까 View와 연결하는건 아니고 ResponseEntity와 연결하기 위한 CarController를 작성한다.
  - 루트 패키지 내에 web을 생성 -> CarController 생성
